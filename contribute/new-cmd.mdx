...
```python
from evadb.parser.parser_visitor._create_statement import CreateTable
...
-  ``statement_to_opr_convertor.py``
   ...
   ```python
      from evadb.optimizer.operators import LogicalCreate
      from evadb.parser.rename_statement import CreateTableStatement
   ```
